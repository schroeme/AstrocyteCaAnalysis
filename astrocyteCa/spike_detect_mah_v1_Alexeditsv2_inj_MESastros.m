function s= spike_detect_mah_v1_Alexeditsv2_inj_MESastros(folderpath,analysis,neurons)
...
% spikes_struct -- structure containing example spikes      
    % spikes_struct=load('C:\Users\Meaneylab\Desktop\Matts m files\Ca Analysis Matts\spikes_matt.mat'); %!!!!!!!!
...
%             xx=1;  %run from here until catch to examine single neuron
            ...
            frames = size(F,1);
            if frames >= 1000
                snr_win=[5:25:200 500 1000];
            else
                snr_win=[5:5:50 75 100];
            end
...
%             if (abs(moment(x,3))>=.0000001)  || SNR>2.5  %%change to 3 - set lower for working through signals
              if SNR>0.5 %.5  %2.5  %%change to 3 - set lower for working through signals
                ...
                %% sort type of signal to determine pk detection threshold  
                    ...
                    if SNR<2.5 %<3 % NEED TO FIND MAX THIS # SHOULD BE ?? %If very noisy
                       ...
                       elseif maxpk/meanpk>2 % if big difference between max pk and average of pks
                               peak_thr=.65*maxpk;  % set larger threshold, so little noisy pks not detected  
                       else % or if ther area a range of little noisy pks
                            peak_thr=.1; % set tiny threshold
                       end
                    else %if not very noisy, set large threshold
                       peak_thr=.40*maxpk;  % proportional to max pk value
                    end
                    peak_thr;
